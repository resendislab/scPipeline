...
join <- function(dirGsea,fls,nm,tag){
  ...
  while (n>0) {
    ...
    DaTa.A <- read.table(file = paste(dirGsea,fls[n],"/",
                                      nm,"_",tag[n],".xls",sep = ""),
                         sep = "\t", header = TRUE)
    ### Selec pathways with FDR<0.05
    DaTa.A <- DaTa.A[DaTa.A[,8]<=0.2,]
...
tag.function <- function(fls,fdr){
  tag <- unlist(lapply(1:length(fls),FUN=function(x) 
    gsub(paste("^",fdr,".\\D*",sep = "" ),"",fls[x])))
  return(tag)
}

wrt.function <- function(Data,pathT,fdr,ltr){
  write.table(Data,file = paste(pathT,"/",fdr,"_",ltr,".csv",sep = ""), 
...
#    nc=choose(n,2)
  ...
  # 
  # }
  #### Comparation by pairs
  comb <- combinations(n, 2, LETTERS[1:n],
                       set=TRUE, repeats.allowed=FALSE)
  N.comb <- length(comb)/2
  for (p in 1:N.comb){
    ### Folder name wheer data are stored
    fdr= paste(mth[i.mth],"_Class_",spc[i.spc],"_",ng[i.ng],"_",
               comb[p,1],"vs",comb[p,2],sep = "")
    fls <- list.files(path=dirGsea,pattern = paste("^",fdr,".",sep = "" ))
    
    tag <- tag.function(fls,fdr)
    
    DaTa <- lapply(1:2, FUN = function(x) 
      join(dirGsea,fls,paste("gsea_report_for_Grp_",
                             comb[p,x],sep = ""),tag))
    lapply(1:2, FUN = function(x) 
     wrt.function(DaTa[[x]],pathT,fdr,comb[p,x]))
