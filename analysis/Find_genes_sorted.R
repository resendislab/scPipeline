...
#                    sep = ""),
#                    sep = "\t")))
genes <- read.table(paste("./results/data/Intersected_genes/",
                          "Genes_A_kmeans_Class_uMAP_6000_(logFC_1.0)",
                          ".txt",
                sep = ""),
         sep = "\t", header = FALSE)
genes.OE <- genes$V1
...
aux.table <- as.data.frame(table(aux))
table.sorted <- aux.table[order(-aux.table$Freq),]
Gs <- intersect(table.sorted$aux,genes.OE)
table.intersectd <- table.sorted[match(Gs,table.sorted$aux),]
write.table(table.intersectd,file = paste("./results/data/DEG_and_GSEA/A_Genes.csv",sep = ""),
                                         row.names = FALSE, col.names = FALSE, sep = ";",
                                         quote = FALSE)
#table.intersectd <- table.intersectd[!is.na(table.intersectd[,2]),]
