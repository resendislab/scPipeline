...
The complete pipeline starts with the fastq files and ends with the differentiallly expresed genes and the interaction maps according pathway enrichment. The [Pipeline.R](Pipeline/pipeline.R) is expresed tue complete pipeline as a R code. The full pipeline is formed by the following modules:
